...
if (array_key_exists("sessionid",$_GET)) {
  ...
  elseif($_SERVER['REQUEST_METHOD'] === 'PATCH') {
    ...
    try {
      ...
      $query = $writeDB->prepare('SELECT sessions.id as sessionid, sessions.user_id as userid, accesstoken, refreshtoken, employee_status, accesstokenexpiry, refreshtokenexpiry from sessions, employees where employees.employee_id = sessions.user_id and sessions.id = :sessionid and sessions.accesstoken = :accesstoken and sessions.refreshtoken = :refreshtoken');
      $query->bindParam(':sessionid', $sessionid, PDO::PARAM_INT);
      $query->bindParam(':accesstoken', $accesstoken, PDO::PARAM_STR);
...
elseif(empty($_GET)) {
  ...
  try {
    ...
    $returned_accesstoken = $row['accesstoken'];
    $returned_refreshtoken = $row['refreshtoken'];
    $returned_useractive = $row['employee_status'];
    $returned_accesstokenexpiry = $row['accesstokenexpiry'];
    $returned_refreshtokenexpiry = $row['refreshtokenexpiry'];
    
    // check if account is active
    if($returned_useractive != 'inactive') {
      $response = new Response();
      $response->setHttpStatusCode(401);
      $response->setSuccess(false);
      $response->addMessage("User account is not active");
  ...
  try {
    ...
    $query = $writeDB->prepare('insert into sessions (user_id, accesstoken, accesstokenexpiry, refreshtoken, refreshtokenexpiry) values (:userid, :accesstoken, date_add(NOW(), INTERVAL :accesstokenexpiryseconds SECOND), :refreshtoken, date_add(NOW(), INTERVAL :refreshtokenexpiryseconds SECOND))');
    // bind the user id
